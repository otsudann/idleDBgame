...
# Quantity / Sell / Buy
var base_fixed_attr = {"qtt": 15, "sell": 0.03, "buy": 0.25}
var base_dynamic_attr = {"qtt": 15, "sell": 0.03, "buy": 0.25}
  ...
  if check:
   for all in all_elements_list:
    for sub in all_elements_list[all]:
      group[all][1][sub] = {"min_qtt": base_dynamic_attr["qtt"], "qtt": 0, "sell": base_dynamic_attr["sell"], "buy": base_dynamic_attr["buy"]}
      base_dynamic_attr["qtt"] = stepify(base_dynamic_attr["qtt"]*multiplier_factor["qtt"], 0.01)
